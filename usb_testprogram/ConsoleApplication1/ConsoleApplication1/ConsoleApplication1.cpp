...
int main(void)
{
	int rc;
		...
		unsigned char *data_read = new unsigned char[4]; //pointer to store data from bulk transfer on inn endpoint.
		...
		r = libusb_bulk_transfer(devh, (0x04), data, 4, &actual, 0); //bulk transfer on out
		if (r == 0 && actual == 4) printf("write_suksess!\n");
