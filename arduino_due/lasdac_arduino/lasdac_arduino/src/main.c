...
#define MAXFRAMESIZE 2500 //in points
...
uint16_t newFrameSize = 0;				//incoming frame total size in bytes
...
uint8_t frame3[MAXFRAMESIZE * 7];		//frame buffer 3
...
uint8_t usbInterruptBuffer[4];						//buffer for usb interrupt transfer (control)
...
void usb_bulk_out_callback(udd_ep_status_t status, iram_size_t length, udd_ep_id_t ep)
{
	if (!newFrameReady)
	{
		...
		cpu_irq_enter_critical();
			if (!playing)
			{
				...
				frameSize = (length - 2); //minus scan rate bytes at the end
				outputSpeed = *(newFrameAddress+newFrameSize);
				newFrameReady = false;
			...
			else
			{
				...
				newFrameSize = (length - 2); //minus scan rate bytes at the end
				outputSpeed = *(newFrameAddress+newFrameSize);
				newFrameReady = true;
	...
	udi_vendor_bulk_out_run(usbBulkBufferAddress, MAXFRAMESIZE * 7, usb_bulk_out_callback);
}
